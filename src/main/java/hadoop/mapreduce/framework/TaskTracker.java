...
import hadoop.mapreduce.type.ReduceTask;
...
/**
 * This process manages tasks on the local machine. It periodically sends
 * heartbeats to the JobTracker, which in return message sends its tasks to run.
 * The Map and reduce tasks should be run in a separate JVM so that the failure
 * of any particular task does not bring down the TaskTracker. The TaskRunner
 * thread takes care of launching and monitoring the the JVM
 * 
 */
public class TaskTracker {
	...
	/**
	 * Send heartbeats to JobTracker with current status and based on response
	 * from the JobTracker, perform required actions
	 */
	public void run() throws Exception {
		...
		while (true) {
			...
			try {
				...
				TaskTrackerHB hb = new TaskTrackerHB(id, getFreeMapperSlots(),
						getFreeReducerSlots(), false, taskSnapshot);
				...
				TaskTrackerHBResponse resp = (TaskTrackerHBResponse) ois
						.readObject();
	...
	private int getFreeMapperSlots() {
		...
		for (TaskManager mgr : taskManagers.values()) {
			if (mgr.getWorkerType() == WorkerType.MAPPER
					&& mgr.getCurrentTask() == null) {
				freeMapSlots++;
			}
		}
		return freeMapSlots;
	...
	private int getFreeReducerSlots() {
		...
		for (TaskManager mgr : taskManagers.values()) {
			if (mgr.getWorkerType() == WorkerType.REDUCER
					&& mgr.getCurrentTask() == null) {
				freeReduceSlots++;
			}
		}
